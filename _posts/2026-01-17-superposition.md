...
Consider multiple species of ants foraging within the same environment. Each species, indexed by $n$, is associated with a characteristic action
...
Consequently, the macroscopic action $h_n$ can be expressed as an integer multiple of this fundamental unit of action,

$$
h_n =  n \, h_0, \quad n \in \mathbb{Z}^+.
$$

The execution of a single Planck's action $h_0$ defines a natural clock tick. This internal clock runs at the fundamental frequency

$$
f_0 = \frac{1}{\tau_0},
$$

where $\tau_0$ is the mean time for a physical system to execute one unit Planck's action.
...
Let $\lambda_n$ denote the mean step length of action $h_n$. The corresponding wavenumber,

$$
k_n =  2 \pi \lambda_n,
$$

measures the spatial frequency of actions performed by species $n$.

The evolution of a multi-species system can then be represented as the superposition of all possible sub-systems,
